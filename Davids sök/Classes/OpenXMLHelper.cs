	...
	public class OpenXMLHelper
	{
		...
		public static DataTable ExcelWorksheetToDataTable(string pathFilename, string worksheetName)
		{
			...
			using (SpreadsheetDocument document = SpreadsheetDocument.Open(pathFilename, false))
			{
				...
				var maxNumOfColumns = 0;//  Iterate through each row of OpenXML data, and store each cell's value in the appropriate slot in our [,] string array.
				foreach (Row row in rows)
				{
					maxNumOfColumns = row.Descendants< Cell >().Count();
...
#if DEBUG
				var maxNumOfRows = dt.Rows.Count;
